...
出力先もコマンド引数で指定できます。未指定の場合、Markdown は生成されますがファイル保存は行いません。
...
### Streamlit Webアプリで実行する

URL 指定スキャンは Streamlit の Web UI からも実行できます。

```bash
streamlit run app.py
```

ブラウザで表示されたフォームに以下を入力し、**スキャン実行** を押します。

- GitHub リポジトリ URL（必須）
- ブランチ / タグ / コミット（任意）
- サブディレクトリ（任意）

完了後、画面上で検知件数・深刻度別件数・検知一覧を確認でき、メモリ上で生成された Markdown レポートをダウンロードできます。Streamlit Webアプリから実行した場合、レポートファイルはローカルに保存されません。
