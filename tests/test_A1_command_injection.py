...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "app.js",
            """
            const cp=require("child_process"); const [cmd=req.query.cmd]=[void 0]; cp.exec(cmd);
            """,
        ),
        (
            "app.js",
            """
            const cp=require('child_process'); let cmd; ({cmd} = req.query); cp.exec(cmd);
            """,
        ),
        (
            "app.js",
            """
            const cp=require('child_process'); let cmd; [cmd] = [req.query.cmd]; cp.exec(cmd);
            """,
        ),
        (
            "app.js",
        ...
        (
            ...
            """
            #!/bin/sh
            if true; then CMD=echo"$1"; eval "$CMD"; fi
            """,
        ),
        (
            "run.sh",
        ...
        (
            "app.js",
            """
            const cp=require("child_process"); const {outer: {cmd}} = {outer: {cmd: req.query.cmd}}; cp.exec(cmd);
            """,
        ),
        (
            "run.sh",
        ...
        (
            "app.js",
            """
            const cp = require("child_process"); const {safe: cmd} = {safe: "date", safe: req.query.cmd}; cp.exec(cmd);
            """,
        ),
        (
            "app.js",
        ...
        (
            "app.js",
            """
            const cp = require("child_process"); const [unsafe, cmd] = [...["date", req.query.cmd]]; cp.exec(cmd);
            """,
        ),
        (
            "run.sh",
        ...
        (
            "app.js",
            """
            const cp = require('child_process'); const holder = {exec: cp.exec}; holder.exec(req.query.cmd);
            """,
        ),
        (
            "app.js",
            """
            const cp = require('child_process'); const my_exec = cp.exec; my_exec(req.query.cmd);
            """,
        ),
        (
            "app.js",
            """
            const cp = require('child_process'); const holder = {exec: cp.exec}; const {exec: my_exec} = holder; my_exec(req.query.cmd);
            """,
        ),
        (
            "run.sh",
        ...
        (
            "app.js",
            """
            const cp = require("child_process"); function run(cmd: string = req.query.cmd) { cp.exec(cmd); }
            """,
        ),
    ],
)
...
def test_js_ignores_safe_cases(tmp_path):
    records = scan_files(
        tmp_path,
        {
            ...
            "app23.js": """
                const cp = require("child_process");
                const {safe: cmd} = {safe: req.query.cmd, ...{safe: "date"}};
                cp.exec(cmd);
            """,
            "app25.js": """
                const cp = require("child_process");
            ...
            "app26.js": """
                const cp = require("child_process");
                function f(cmd = req.query.cmd) { return cmd; }
                const cmd = "date";
                cp.exec(cmd);
            """,
        },
