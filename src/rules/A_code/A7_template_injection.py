        ...
        for py_file in self._iter_py_files(target):
            ...
            for node in ast.walk(tree):
                if not isinstance(node, ast.Call):
                    continue
                callee = self._resolve_name(self._full_name(node.func), aliases)
                if callee not in self._TEMPLATE_SINKS:
                    continue
                template_arg = node.args[0] if node.args else None
                if template_arg and self._is_external_expr(
                    template_arg, tainted, aliases
                ):
                    records.append(
                        RiskRecord(
                            ...
                            message="外部入力をテンプレート本文として評価しています（Template Injectionの恐れ）",
                        )
                    )
