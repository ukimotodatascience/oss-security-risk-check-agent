        ...
        for py_file in self._iter_py_files(target):
            ...
            for node in ast.walk(tree):
                if not isinstance(node, ast.Call):
                    continue
                callee = self._resolve_name(self._full_name(node.func), aliases)
                if callee not in self._PY_XSS_SINKS:
                    continue
                arg0 = node.args[0] if node.args else None
                if arg0 and self._is_external_expr(arg0, tainted, aliases):
                    records.append(
                        RiskRecord(
                            ...
                            message="外部入力を未エスケープでHTML/テンプレート出力へ渡しています",
                        )
                    )

        js_external = re.compile(
            r"req\.(?:query|body|params)|location\.(?:search|hash)|document\.URL|window\.name"
        )
        js_sink = re.compile(
                ...
                if js_sink.search(line) and js_external.search(line):
                    records.append(
                        RiskRecord(
                            ...
                            file_path=rel_path,
                            line=i,
                            message="外部入力をDOMのHTML挿入APIに渡しています（XSSの恐れ）",
                        )
                    )
