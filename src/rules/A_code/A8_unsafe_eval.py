        ...
        for py_file in self._iter_py_files(target):
            ...
            for node in ast.walk(tree):
                if not isinstance(node, ast.Call):
                    continue
                callee = self._resolve_name(self._full_name(node.func), aliases)
                if callee not in self._DANGEROUS_EVAL_CALLS:
                    continue
                if not node.args:
                    continue

                arg0 = node.args[0]
                if self._is_external_expr(arg0, tainted, aliases):
                    severity = (
                        Severity.MEDIUM
                        if callee == "ast.literal_eval"
                        else Severity.HIGH
                    )
                    records.append(
                        RiskRecord(
