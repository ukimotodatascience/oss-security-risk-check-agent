...
class JsTsCommandInjectionDetector(JsTsSinkMixin, JsTsSourceMixin):
    ...
    def _evaluate_js_ts_file_with_tree_sitter(
        ...
        for node in nodes:
            ...
            if node_type in {"variable_declarator", "assignment_expression"}:
                ...
                if left is not None and right is not None:
                    ...
                    if re.fullmatch("[A-Za-z_$][\\w$]*", left_text):
                        ...
                        if (
                            right_clean == "shelljs.exec"
                            or right_clean == "shelljs"
                            or f"{right_clean}.exec" in shelljs_sinks
                            or (
                                right_clean.endswith(".exec")
                                and right_clean.rpartition(".")[0] in shelljs_sinks
                            )
                            or right_clean in shelljs_sinks
                        ):
                            shelljs_sinks.add(left_text)
                    if re.fullmatch("[A-Za-z_$][\\w$]*", left_text) and is_top_level:
                        if self._js_options_enable_shell(right_clean):
                            ...
                            shell_true_option_names.discard(left_text)
                    if re.fullmatch(
                        "[A-Za-z_$][\\w$]*", left_text
            ...
            if (
                direct_shelljs_call or direct_child_process_call
            ) and "require(" in callee:
                if self._is_shadowed_child_process_sink(
                    node, "require", src_bytes
                ) or self._is_shadowed_shelljs_sink(node, "require", src_bytes):
                    direct_shelljs_call = False
                    direct_child_process_call = None
            ...
            sink_api = self._child_process_api_for_callee(
                normalized_callee, valid_child_sinks
            )
            is_known_sink = (
    ...
    def _evaluate_js_ts_file(self, file_path: Path, target: Path) -> List[RiskRecord]:
        ...
        for i, stripped in statements:
            ...
            is_scoped_statement = bool(
                ...
                or re.match(
                    r"\s*(?:if|else|for|while|do|switch|try|catch|finally)\b", stripped
                )
            )
            ...
            if assignment:
                ...
                option_rhs_clean = option_rhs.rstrip(";").strip()
                if self._js_options_enable_shell(option_rhs_clean):
    ...
    @staticmethod
    def _is_top_level_node(node) -> bool:
        """ノードがファイルトップレベル（関数/アロー/メソッドの外）にあるかを判定する。"""
        _SCOPE_TYPES = {
        ...
        while ancestor is not None:
            if getattr(ancestor, "type", "") in _SCOPE_TYPES:
                return False
            ancestor = getattr(ancestor, "parent", None)
        return True

    @staticmethod
    def _is_sink_binding_out_of_scope(
        node, binding_name: str, src_bytes: bytes, is_sink_decl_fn
    ) -> bool:
        """Check if binding_name has a valid sink declaration at top-level or in an enclosing scope of node."""
        full_source = src_bytes.decode("utf-8", errors="ignore")
        top_level_source = (
        ...
        while curr is not None:
            if getattr(curr, "type", "") in {
                "function_declaration",
                "function_expression",
                "arrow_function",
                "method_definition",
            }:
                scope_prefix = src_bytes[curr.start_byte : node.start_byte].decode(
                    "utf-8", errors="ignore"
                )
                if is_sink_decl_fn(scope_prefix, binding_name):
                    return False
    ...
    def _is_specific_child_process_declaration(text: str, name: str, api: str) -> bool:
        if api:
            if (
                ...
                or re.search(
                    rf"\b(?:const|let|var)\s*\{{[^}}]*\b{re.escape(api)}\s*:\s*{re.escape(name)}\b[^}}]*\}}\s*=\s*require\s*\(\s*['\"](?:node:)?child_process['\"]\s*\)",
                    text,
                )
                or re.search(
                    rf"\b(?:const|let|var)\s+{re.escape(name)}\b[^;=]*=\s*"
                    rf"(?:require\s*\(\s*['\"](?:node:)?child_process['\"]\s*\)\.{re.escape(api)}|"
    ...
    def _is_shadowed_child_process_sink(node, callee: str, src_bytes: bytes) -> bool:
        ...
        def check_decl(text: str, name: str) -> bool:
            if api:
                return (
                    JsTsCommandInjectionDetector._is_specific_child_process_declaration(
                        text, name, api
                    )
                )
            return JsTsCommandInjectionDetector._is_child_process_declaration_binding(
        ...
        if JsTsCommandInjectionDetector._is_sink_binding_out_of_scope(
            node,
            binding_name,
            src_bytes,
            check_decl,
    ...
    def _is_child_process_declaration_binding(text: str, name: str) -> bool:
        if re.search(
            ...
            r"\(*\s*await\s+import\s*\(\s*['\"](?:node:)?child_process['\"]|child_process\.(?:exec|execSync|spawn|spawnSync|execFile|execFileSync|fork))",
            text,
        ):
            return True
        if re.search(
            ...
            return True
        if re.search(
            rf"\bimport\s+[\s\S]*?\b{re.escape(name)}\b[\s\S]*?\bfrom\s*['\"](?:node:)?child_process['\"]",
            text,
        ) or re.search(
            rf"\bimport\s+{re.escape(name)}\s*=\s*require\s*\(\s*['\"](?:node:)?child_process['\"]\s*\)",
            text,
        ):
            return True
        for match in re.finditer(
            rf"\b(?:const|let|var)\s+{re.escape(name)}\b[^;=]*=\s*([A-Za-z_$][\w$]*)\s*\.\s*(?:exec|execSync|spawn|spawnSync|execFile|execFileSync|fork)\b",
            text,
        ):
            mod_name = match.group(1)
            if mod_name == "child_process" or re.search(
                rf"\bimport\s+[\s\S]*?\b{re.escape(mod_name)}\b[\s\S]*?\bfrom\s*['\"](?:node:)?child_process['\"]|"
                rf"\b(?:const|let|var)\s+{re.escape(mod_name)}\b[^;=]*=\s*require\s*\(\s*['\"](?:node:)?child_process['\"]\s*\)",
                text,
            ):
                return True
        return False
    ...
    def _is_known_third_party_shell_sink(text: str, shelljs_sinks: Set[str]) -> bool:
        return bool(
            ...
            or (
                ...
                and JsTsCommandInjectionDetector._contains_unshadowed_shelljs_call(
                    text, "require"
                )
            )
    ...
    def _contains_unshadowed_shelljs_call(text: str, name: str) -> bool:
        ...
        has_local_declaration = bool(
            re.search(
                rf"\b(?:const|let|var|function|class)\s+{re.escape(binding_name)}\b",
                prefix,
            )
            or re.search(
                rf"\((?:[^)]*,\s*)?{re.escape(binding_name)}\b[^)]*\)\s*=>|"
                rf"\bfunction\b[^(]*\((?:[^)]*,\s*)?{re.escape(binding_name)}\b[^)]*\)",
                prefix,
            )
        )
    ...
    def _is_shadowed_shelljs_sink(node, callee: str, src_bytes: bytes) -> bool:
        ...
        if JsTsCommandInjectionDetector._has_non_shelljs_reassignment(
            ...
            return True
        if scope is None:
            full_prefix = src_bytes[: node.start_byte].decode("utf-8", errors="ignore")
            if JsTsCommandInjectionDetector._is_shelljs_declaration_binding(
                full_prefix, binding_name
            ) and not JsTsCommandInjectionDetector._is_shelljs_declaration_binding(
                prefix, binding_name
            ):
                return True
    ...
    def _is_shelljs_declaration_binding(text: str, name: str) -> bool:
        if re.search(
            ...
            return True
        if re.search(
            rf"\bimport\s+[\s\S]*?\b{re.escape(name)}\b[\s\S]*?\bfrom\s*['\"]shelljs['\"]",
            text,
        ) or re.search(
            rf"\bimport\s+{re.escape(name)}\s*=\s*require\s*\(\s*['\"]shelljs['\"]\s*\)",
            text,
        ):
            return True
        for match in re.finditer(
